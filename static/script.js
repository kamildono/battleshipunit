...
function handleOwnCellClick(x,y){ if(gameStarted)return; const ship=findShipAt(x,y); ship?removeShip(ship):placeShip(x,y);}
...
// function updateShots(data){ const own=document.getElementById('ownBoard').children; const opp=document.getElementById('opponentBoard').children; for(let c of own) c.classList.remove('hit','miss','sunk', 'forbidden'); for(let c of opp) c.classList.remove('hit','miss','sunk', 'forbidden'); data.opponent_shots.forEach(s=>{ let i=s.y*10+s.x; if(opp[i]) opp[i].classList.add(s.hit?'hit':'miss'); }); data.own_shots.forEach(s=>{ let i=s.y*10+s.x; if(own[i]) own[i].classList.add(s.hit?'hit':'miss'); }); if(data.sunk_ships){ data.sunk_ships.forEach(ship=>ship.forEach(({x,y})=>{ let i=y*10+x; if(opp[i]){ opp[i].classList.remove('hit'); opp[i].classList.add('sunk'); } })); } }
...
function pollState() {
  setInterval(() => {
    ...
    fetch(`/api/get_state/${player}`)
      .then(r => r.json())
      .then(data => {
        // если сервер сбросил игру — оба ready должны стать false
        ...
        // после старта боя прячем селекторы
        if (gameStarted) {
          document.getElementById('shipSelectors').style.display = 'none';
        }
